...
def log(msg: str, logger: Logger, level: int = LogLevel.INFO) -> None:
    """Log the message to the given Logger at the given level"""
    if level >= LogLevel.ERROR:
        msg = f"\x1b[1;31m{msg}\x1b[0m"
    elif level >= LogLevel.WARNING:
        msg = f"\x1b[1;33m{msg}\x1b[0m"
    elif level < LogLevel.DEBUG:
        msg = f"\x1b[1;30m{msg}\x1b[0m"

    logger.logger.log(msg=msg, level=level)


@contextlib.contextmanager
def time_section(task: str, level: int = LogLevel.INFO) -> Iterator[None]:
...
def _setup_nfs(
    ...
    log(f"{d=} {t=} m={r}^{k}", logger=Logger.MISC)
    ...
    assert not (math.gcd(4, d) == 4 and is_nth_power(4 * t, n=4)), "f is reducible"
    ...
    log(f"{alpha_i=}", logger=Logger.SEARCH, level=LogLevel.DEBUG)
...
def _search_for_elements(
    ...
    for gamma in itertools.product(*(range(-bound, bound + 1) for bound in s_bounds)):
        ...
        for prime_index, (p, c_list, power) in enumerate(
            ...
            for ideal_index, c in enumerate(c_list):
                if (
                    sum((s_i * pow(c, i, p)) % p for i, s_i in enumerate(gamma)) % p
                    == 0
                ):
                    break
            else:
    ...
    for prime_index, (current_multipliers, current_generators) in enumerate(
        ...
        ), f"Too large multiplier at {prime_index=}"
        assert not any(
            g is None for g in current_generators
        ), f"Missing generator at {prime_index=}"
    ...
    for i in itertools.count(1):
        if len(found_units) <= params.l:
            break

        power_bound = 2**i

        log(
            f"Eliminating linear dependencies up to the power {power_bound}",
            logger=Logger.SEARCH,
        ...
        for u1 in found_units.copy():
            if u1 not in found_units:
                # Already been removed
                continue

            span = tuple(
                map_to_integer(compute_power(u1, exponent, params))
                for exponent in range(-power_bound, power_bound + 1)
            )
            for new_unit in span:
                if new_unit != u1 and new_unit in found_units:
                    found_units.remove(new_unit)

                # u0 = -1
                shifted = negate(new_unit)  # = multiply(new_unit, u0, params)
                if shifted in found_units:
                    found_units.remove(shifted)

            if params.l == 1:  # noqa: E741
                # Don't look at pairs of units when we only want 1
                continue
    ...
    ), f"Did not eliminate enough units {len(found_units)}>{params.l}"

    assert (
        len(found_units) == params.l
    ), f"Did not find a sufficient set of units {len(found_units)}<{params.l}"
